...
def parse_skill_tree(tree_text: str) -> Dict[str, List[str]]:
    """
    Parse the hierarchical skill tree from text format.
    ...
    Returns:
        Dict mapping parent skill -> list of child skills (full paths)
    """
    hierarchy = {}
    lines = tree_text.strip().split('\n')
    stack = []  # Stack of (indent_level, skill_path)
    ...
    for line in lines:
        ...
        # Build full path: "microsoft sql server reporting services"
        if stack:
...
def build_skill_pairs(hierarchy: Dict[str, List[str]]) -> Tuple[List[Tuple[str, str]], List[Tuple[str, str]]]:
    ...
    # Get all skills (flatten hierarchy)
    all_skills = set()
    ...
    for parent, children in hierarchy.items():
        for child in children:
            ...
            # Also add relationship between parent and grandchildren
            if child in hierarchy:
                for grandchild in hierarchy[child]:
    ...
    # First, identify root-level skills (not children of any parent)
    root_skills = []
    all_children = set()
    for parent, children in hierarchy.items():
    ...
    # CRITICAL FIX: Ensure ALL root-level skills are negative pairs
    # This prevents unrelated skills like "JIRA" and "Scala" from becoming similar
    ...
    # Then add more negative pairs from all skills (with higher ratio)
    max_negatives = len(positive_pairs) * 5  # INCREASED from 2:1 to 5:1 ratio
...
def create_training_examples(positive_pairs: List[Tuple[str, str]], 
    ...
    # Positive pairs (label=1.0 for high similarity)
    ...
    # Negative pairs (label=0.0 for low similarity)
...
def main():
    ...
    # Step 1: Parse skill tree
    print("\n[1/5] Parsing skill hierarchy...")
    ...
    print("\n[4/5] Loading base embedding model...")
    ...
    # Use ContrastiveLoss for hierarchical relationships
    # FIXED: Increased margin from 0.5 to 1.0 to better separate unrelated skills
    train_loss = losses.ContrastiveLoss(
        model,
        distance_metric=losses.SiameseDistanceMetric.COSINE_DISTANCE,
        margin=1.0  # INCREASED from 0.5 to 1.0 - stronger separation between unrelated skills
    )
